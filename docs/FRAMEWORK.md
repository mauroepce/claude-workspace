...
**Match on word boundaries.** The difference between signal and noise, and the obvious implementation is wrong: `\b` does not help, because a hyphen is a non-word character, so `tg-app\b` still matches `tg-app-dev`. A PCRE lookaround does the job exactly and about ten times faster, but `git grep` is not always built with PCRE, so the join detects the engine and falls back to ERE that captures the neighbouring character and strips it. Before boundaries, a real run produced two entirely false edges (a handle `tg-gateway` matching an unrelated `tg-gateway-ingress`, `tg-ms` matching a bucket named `tg-msa-dbs`) and inflated a real one threefold on GCP project names; after, all three resolved. The fallback has one known gap, measured at 26 missed matches in 6,148: `grep -o` does not overlap, so adjacent occurrences lose their boundary.
