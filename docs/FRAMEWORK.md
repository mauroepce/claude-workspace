...
**Match on word boundaries.** The difference between signal and noise, and the obvious implementation is wrong: `\b` does not help, because a hyphen is a non-word character, so `tg-app\b` still matches `tg-app-dev`. A PCRE lookaround is exact and about twelve times faster (2 seconds against 25 on six repos), but `git grep` is not always built with PCRE, so the join probes for it — with a lookaround, since a plain pattern does not force that code path and would report false success — and falls back to ERE that captures the neighbouring character and strips it. Before boundaries, a real run produced two entirely false edges (a handle `tg-gateway` matching an unrelated `tg-gateway-ingress`, `tg-ms` matching a bucket named `tg-msa-dbs`) and inflated a real one threefold on GCP project names; after, all three resolved and a later PCRE run matched an independent scan exactly. The fallback keeps one known gap, measured at 26 missed matches in 6,148: `grep -o` does not overlap, so adjacent occurrences lose their boundary.
...
**Separate vocabulary from addresses.** The handles file carries two lines that do different jobs. `serves:` holds qualified strings a peer would literally contain — package names, env vars, deployable names — and only these derive edges. `aka:` holds bare product names like `langgraph` or `piston`, which map a task phrase onto a repo but never draw an edge.

That split was bought with a failed experiment. Boundaries make short bare names *syntactically* safe, so a run put them in `serves:`. The result: fourteen new edge rows and **zero new true dependencies**. Six were backed by code, six were real pairs whose only evidence was prose, one recorded where code had been copied from, and one was a false pair pointing the wrong way — a comment describing a repo's *consumer* became an edge *to* it. One bare name supplied 73% and 81% of the hits on two pairs it did not discover. Word boundaries fix substring noise; they cannot tell a call from a sentence, and prose is exactly where product names live.

Applying the split removed twelve of those fourteen rows, including the false pair, and cost one real edge — which turned out to be the most useful thing it could have done. A test-analytics repo depended on another repo not by calling it but by reading what it *produces*: its GitHub slug and a bucket its CI writes. No service address existed to find, so the relationship had been visible only through a bare product name, by accident. Artifact names are now a source of their own, which is what that edge needed all along.
