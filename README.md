...
- **`/checkpoint`** — force-update the active subproject's `STATUS.md` based on the current session.

> **STATUS.md is maintained by Claude, not by you.** The root `CLAUDE.md` (generated by `/setup`) instructs Claude to proactively update each subproject's `STATUS.md` at the end of every productive session — completed TODOs, decisions made (with the *why*), new blockers, etc. You don't have to remember. If you want to force an update mid-session use `/checkpoint`. You can still edit `STATUS.md` directly when you prefer.
